...
def test_settings_load_without_env_crash():
    ...
    loaded = settings.load_settings("1.16.5")
    assert loaded.app_version == "1.16.5"
    assert loaded.session_max_age_seconds > 0


def test_version_unified():
    assert (ROOT / "VERSION.txt").read_text().strip() == "1.16.5"
    main_text = (ROOT / "main.py").read_text()
    assert 'APP_VERSION = "1.16.5"' in main_text
    assert 'APP_BUILD = "Fjord VI 1.16.5"' in main_text
