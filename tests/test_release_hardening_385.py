...
def test_version_385_unified():
    assert (ROOT / "VERSION.txt").read_text(encoding="utf-8").strip() == "3.8.5"
    assert 'APP_VERSION = "3.8.5"' in MAIN
    assert 'APP_BUILD = "Fjord VI 3.8.5"' in MAIN
    assert 'RELEASE_LABEL = "Fjord VI · v3.8.5"' in MAIN
